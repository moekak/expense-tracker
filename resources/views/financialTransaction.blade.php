    ...
    <div class="expense_area">
        <form action="{{route("transaction.store")}}" style="width: 36%" class="create-expense-form" method="post">
            ...
            @error('financial_category_id')
                <p class="error_txt error_txt_marginT">{{$errors->first("financial_category_id")}}</p>
            @enderror
            <select disabled  name="transaction_category_id" id="" class="js_select_financialCategory input_field input select_field" style="cursor: pointer">
                <option class="default_option" disabled value=""  {{ old('transaction_category_id') ? '' : 'selected' }}>Select category</option>
                @foreach ($transaction_categories as $transaction_category)
                    <option class="js_transaction_categoty" data-id="{{$transaction_category->financial_category_id}}" value="{{$transaction_category->id}}" @if ($transaction_category->id == old("transaction_category_id"))  selected @endif>{{$transaction_category->transaction_category_name}}</option>
                @endforeach
            </select>
            @error('transaction_category_id')
                    <p class="error_txt error_txt_marginT">{{$errors->first("transaction_category_id")}}</p>
                @enderror
            <select name="credit_card_id" id="" class="input_field input select_field" style="cursor: pointer">
             ...
             @error('credit_card_id')
                <p class="error_txt error_txt_marginT">{{$errors->first("credit_card_id")}}</p>
            @enderror
